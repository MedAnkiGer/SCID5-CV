...
def generate_pdf(session: dict, questions: dict, output_path: Path) -> Path:
    ...
    for disorder_key, disorder_data in questions.get("disorders", {}).items():
        ...
        for crit_id, result in explored_criteria.items():
            ...
            pdf.cell(0, 6, _safe_text(f"{crit_id}: {desc}"), new_x="LMARGIN", new_y="NEXT")
            ...
            if transcript:
                ...
                pdf.multi_cell(0, 4, _safe_text(f'Patient: "{transcript[:500]}"'))

            # Rationale
            rationale = result.get("rationale", "")
            if rationale:
                pdf.set_font("Helvetica", "", 8)
                pdf.multi_cell(0, 4, _safe_text(f"AI Rationale: {rationale[:500]}"))

            # Clarification transcript if present
            clarification = result.get("clarification_transcript")
            if clarification:
                pdf.set_font("Helvetica", "I", 8)
                pdf.multi_cell(0, 4, _safe_text(f'Clarification: "{clarification[:500]}"'))
    ...
    if unresolved_items:
        ...
        for crit_id, result in unresolved_items:
            ...
            if clarifying:
                pdf.set_font("Helvetica", "I", 8)
                pdf.multi_cell(0, 4, _safe_text(f"Suggested question: {clarifying}"))
            pdf.ln(2)
